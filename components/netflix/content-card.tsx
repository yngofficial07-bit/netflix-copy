...
export function ContentCard({ item, isLarge = false, index, animationDelay = 0, onOpenModal, onPlay, isLiked, onToggleLike, isInMyList, onToggleMyList }: ContentCardProps) {
  const [isHovered, setIsHovered] = useState(false)
  const [imageError, setImageError] = useState(false)
  const [isTouch, setIsTouch] = useState(false)
  const cardRef = useRef<HTMLDivElement>(null)
  const { settings } = useSettings()
  const inList = isInMyList?.(item.id) || false
  ...
  const handlePlay = (e?: React.MouseEvent) => {
    ...
    window.location.href = `https://auraflix-junction.vercel.app/index.html?id=${id}&type=${type}`
  }
  ...
  return (
    <motion.div
      ...
      transition={{ duration, delay: animationDelay, ease: [0.25, 0.1, 0.25, 1] }}
    ...
    >
      ...
      >
        ...
        <div className={`relative overflow-hidden ${getBorderRadius()}`}>
          <Image
            src={imageError ? "https://via.placeholder.com/220x124?text=No+Image" : (isLarge ? item.imageUrl : (item.backdropUrl || item.imageUrl))}
            alt={item.title}
            width={isLarge ? 260 : 220}
            height={isLarge ? 390 : 124}
            className={`object-cover transition-transform duration-300 ease-out w-full ${isLarge ? "aspect-[2/3]" : "aspect-video"} ${isHovered && !settings.reducedMotion ? "scale-110" : ""}`}
        ...
        <AnimatePresence>
          {isHovered && !settings.reducedMotion && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              transition={{ duration: duration * 0.6, ease: [0.25, 0.1, 0.25, 1] }}
              ...
              <div className="p-3 md:p-4 space-y-3">
                <div className="flex items-center gap-1.5 md:gap-2">
                  ...
                  <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.95 }}
                    ...
                    className={`h-8 w-8 md:h-9 md:w-9 rounded-full border-2 flex items-center justify-center transition-all ${inList ? "bg-white/20 border-white" : "border-gray-500 hover:border-white"}`}
                  >
                    {inList ? <Check className="h-3.5 w-3.5 text-white" /> : <Plus className="h-3.5 w-3.5 text-white" />}
                  </motion.button>

                  {/* Like */}
                  <motion.button whileHover={{ scale: 1.15 }} whileTap={{ scale: 0.95 }}
                    onClick={e => { e.stopPropagation(); onToggleLike?.(item) }}
                    className={`h-8 w-8 md:h-9 md:w-9 rounded-full border-2 flex items-center justify-center transition-all ${liked ? "border-white" : "border-gray-500 hover:border-white"}`}
                  >
                    <ThumbsUp className={`h-3.5 w-3.5 ${liked ? "text-white fill-white" : "text-white"}`} />
                  </motion.button>
