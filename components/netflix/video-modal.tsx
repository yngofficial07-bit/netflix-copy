...
export function VideoModal({ item, isOpen, onClose, isInMyList, onToggleMyList, isLiked, onToggleLike, onPlay }: VideoModalProps) {
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 md:p-8">
                  ...
                  <div className="flex flex-wrap items-center gap-2 md:gap-3">
                    ...
                    <motion.button
                      ...
                      className={`p-2.5 md:p-3 rounded-full border-2 transition-all ${
                        liked ? "bg-white/20 border-white" : "border-gray-400 hover:border-white bg-black/40"
                      }`}
                    >
                      <ThumbsUp className={`h-4 w-4 md:h-5 md:w-5 ${liked ? "text-white fill-white" : "text-white"}`} />
                    </motion.button>
                      ...
                      {episodes.slice(0, showMoreEpisodes ? 10 : 4).map((ep, i) => (
                        <motion.div
                          ...
                          onClick={() => {
                            onPlay?.(item)
                            const id = item.tmdbId || item.id
                            window.location.href = `https://auraflix-junction.vercel.app/index.html?id=${id}&type=tv&s=1&e=${ep.number}`
                          }}
