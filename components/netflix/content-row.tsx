...
interface ContentRowProps {
  ...
  onExploreAll?: (rowKey: string | undefined, title: string, items: ContentItem[]) => void
  rowKey?: string
}

export function ContentRow({ title, items, isLarge = false, delay = 0, onOpenModal, onPlay, isLiked, onToggleLike, isInMyList, onToggleMyList, onExploreAll, rowKey }: ContentRowProps) {
  ...
  return (
    <motion.div
      initial={settings.reducedMotion ? {} : { opacity: 0, y: 30 }}
      animate={hasAnimated ? { opacity: 1, y: 0 } : {}}
      transition={{ duration: duration * 1.5, delay: delay * 0.1, ease: [0.25, 0.1, 0.25, 1] }}
    ...
    >
      ...
      <motion.div
        ...
        transition={{ duration: duration * 1.2, delay: delay * 0.1 + 0.1, ease: [0.25, 0.1, 0.25, 1] }}
      >
        <h2 className="text-base md:text-xl lg:text-2xl font-bold text-white">{title}</h2>
        <button
          type="button"
          onClick={() => onExploreAll?.(rowKey, title, items)}
          className="text-xs md:text-sm font-medium flex items-center gap-1 cursor-pointer hover:underline transition-opacity opacity-70 hover:opacity-100"
